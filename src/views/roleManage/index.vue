	...
	export default {
      ...
      getMenuList(){
        ...
        getMenuList(data).then(res=>{
          this.menus=res.result.data.sort(this.createCompare('order_num'));
        })
      },
			//删除
			handleDel(index, row) {
			...
			editSubmit() {
				...
				data.menu_id=this.getMenuIds().join(',');
      ...
      getMenuIds() {
        ...
        return this.$refs.menuAddTree.getCheckedKeys();
      },
      /**
       * (keys, leafOnly) 接收两个参数，1. 勾选节点的 key 的数组 2. boolean 类型的参数，若为 true 则仅设置叶子节点的选中状态，默认值为 false
       */
      setMenuIds(keys) {
        this.$refs.menuAddTree.setCheckedKeys(keys, false);
        // this.$refs.menuAddTree.setCheckedNodes(keys, true);
      }
		},
