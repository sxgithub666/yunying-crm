	...
	<section>
			...
			<el-form :model="editForm" label-width="120px" :rules="editFormRules" ref="editForm">
				...
				<div class="flex">
					<el-form-item label="客户所在区域" prop="customer_region">
						<area-cascader disabled v-if="showArea" v-model="addForm.customer_region" :level="0" type="text" :data="pcaa"></area-cascader>
					</el-form-item>
		...
		<el-dialog title="新增" width="53%" :visible.sync="addFormVisible" :close-on-click-modal="false">
			<el-form :model="addForm" label-width="120px" :rules="editFormRules" ref="addForm">
				...
				<div class="flex">
					<el-form-item label="客户所在区域" prop="customer_region">
						<area-cascader :disabled="disabled" v-if="showArea" v-model="addForm.customer_region" :level="0" type="text" :data="pcaa"></area-cascader>
					</el-form-item>
					<el-form-item style="width:50%" label="收款时间" prop="pay_time">
						<el-date-picker :disabled="disabled" v-model="addForm.pay_time" @change="getEditTime" value-format="yyyy-MM-dd" type="date" placeholder="选择收款日期">
						</el-date-picker>
					</el-form-item>
				</div>
				<div class="flex">
		...
		data() {
			return {
				...
				editFormRules: {
					...
					// need_data: [{ required: true, message: '请输入申请号码材料', trigger: 'change,blur'}],
			...
			addSubmit() {
				this.$refs.addForm.validate((valid) => {
					if (valid) {
						...
						this.addForm.customer_region=this.addForm.customer_region?this.addForm.customer_region.join('/'):'';
